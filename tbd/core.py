# -*- coding: utf-8 -*-
# tbd - a version control system built on top of Git
# Licensed under MIT
...
class TbdError(Exception): pass

class NotInRepoError(TbdError): pass

class BranchIsCurrentError(TbdError): pass

class ApplyFailedError(TbdError): pass
...
TBD_STATUS_UNTRACKED = 1
TBD_STATUS_TRACKED = 2
TBD_STATUS_IGNORED = 3
...
def init_repository(url=None):
  """Creates a new tbd's repository in the cwd.
  ...
  try:
    ...
    raise TbdError('You are already in a tbd repository')
...
class Repository(object):
  ...
  Attributes:
    path: absolute path to the tbd's dir (the .git dir).
  ...
  def __init__(self):
    ...
    try:
      ...
      raise NotInRepoError('You are not in a tbd\'s repository')
  ...
  def merge_base(self, b1, b2):
    try:
      ...
      raise TbdError('No common commit found between {0} and {1}'.format(b1, b2))

  def _fuse_commits_fp(self, b):
    return os.path.join(
        self.path, 'TBD_FUSE_CIS_{0}'.format(b.branch_name.replace('/', '_')))
  ...
  def current_branch(self):
    if self.git_repo.head_is_detached:
      b = self.git_repo.lookup_reference('TBD_FUSE_ORIG_HEAD').resolve()
    else:
  ...
  def switch_current_branch(self, dst_b, move_over=False):
    ...
    au_fp = lambda b: os.path.join(
        self.path, 'TBD_AU_{0}'.format(b.branch_name.replace('/', '_')))
      ...
      if b.merge_in_progress or b.fuse_in_progress:
        ...
        if b.merge_in_progress:
          ...
          ref_info = {
              ...
              'TBD_FUSE_ORIG_HEAD': str(self._ref_target('TBD_FUSE_ORIG_HEAD')),
              'CHERRY_PICK_HEAD': str(self._ref_target('CHERRY_PICK_HEAD'))
              }
          self._ref_rm('TBD_FUSE_ORIG_HEAD')
    ...
    def restore(b):
      ...
      if len(split_msg) == 1:  # No op to restore
        ...
        if 'TBD_FUSE_ORIG_HEAD' in ref_info:  # fuse
          ...
          self._ref_create('TBD_FUSE_ORIG_HEAD', ref_info['TBD_FUSE_ORIG_HEAD'])
        else:  # merge
...
class RemoteCollection(object):
  ...
  def __init__(self, git_remote_collection, tbd_repo):
    self.git_remote_collection = git_remote_collection
    self.tbd_repo = tbd_repo

  def __len__(self):
    return len(self.git_remote_collection)

  def __iter__(self):
    return (Remote(r, self.tbd_repo) for r in self.git_remote_collection)

  def __getitem__(self, name):
    return Remote(self.git_remote_collection.__getitem__(name), self.tbd_repo)
...
class Remote(object):
  ...
  def __init__(self, git_remote, tbd_repo):
  ...
  def create_branch(self, name, head):
    ...
    # push
    tmp_b = self.tbd_repo.create_branch('tbd_tmp_ref', head)
    try:
  ...
  def lookup_branch(self, branch_name):
    ...
    git_branch = self.tbd_repo.git_repo.lookup_branch(
        self.git_remote.name + '/' + branch_name, pygit2.GIT_BRANCH_REMOTE)
    return RemoteBranch(git_branch, self.tbd_repo)
  ...
  def create_tag(self, name, commit):
    ...
    # push
    tmp_t = self.tbd_repo.create_tag('tbd_tmp_ref', commit)
    try:
  ...
  def lookup_tag(self, tag_name):
    ...
    commit = self.tbd_repo.git_repo.get(commit_id).peel(pygit2.GIT_OBJ_COMMIT)
...
class RemoteBranch(object):
  ...
  def __init__(self, git_branch, tbd_repo):
  ...
  def history(self, reverse=False):
    return walker(self.tbd_repo.git_repo, self.target, reverse=reverse)

  def _update(self):
    git.fetch(self.remote_name, self.branch_name)
    self.git_branch = self.tbd_repo.git_repo.lookup_branch(
...
class Branch(object):
  ...
  def upstream(self):
    ...
    try:
      ...
      return RemoteBranch(git_upstream, self.tbd_repo)
    except ValueError:  # Upstream is a local branch
      return Branch(git_upstream, self.tbd_repo)
  ...
  def head(self, new_head):
    self.tbd_repo.git_repo.reset(new_head, pygit2.GIT_RESET_SOFT)
  ...
  @property
  def is_current(self):
    return self.tbd_repo.current_branch.branch_name == self.branch_name
  ...
  def _index(self):
    ...
    return Index(self.tbd_repo.git_repo.index)

  _st_map = {
    # git status: tbd status, exists_at_head, exists_in_wd, modified, conflict

    pygit2.GIT_STATUS_CURRENT: (TBD_STATUS_TRACKED, True, True, False, False),
    pygit2.GIT_STATUS_IGNORED: (TBD_STATUS_IGNORED, False, True, True, False),
    pygit2.GIT_STATUS_CONFLICTED: (TBD_STATUS_TRACKED, True, True, True, True),

    ### WT_* ###
    pygit2.GIT_STATUS_WT_NEW: (TBD_STATUS_UNTRACKED, False, True, True, False),
    pygit2.GIT_STATUS_WT_MODIFIED: (TBD_STATUS_TRACKED, True, True, True, False),
    pygit2.GIT_STATUS_WT_DELETED: (TBD_STATUS_TRACKED, True, False, True, False),

    ### INDEX_* ###
    pygit2.GIT_STATUS_INDEX_NEW: (TBD_STATUS_TRACKED, False, True, True, False),
    pygit2.GIT_STATUS_INDEX_MODIFIED: (
      TBD_STATUS_TRACKED, True, True, True, False),
    pygit2.GIT_STATUS_INDEX_DELETED: (
      TBD_STATUS_TRACKED, True, False, True, False),

    ### WT_NEW | INDEX_* ###
    # WT_NEW | INDEX_NEW -> can't happen
    # WT_NEW | INDEX_MODIFIED -> can't happen
    # WT_NEW | INDEX_DELETED -> could happen if user broke tbd layer (e.g., did
    ...
    pygit2.GIT_STATUS_WT_MODIFIED | pygit2.GIT_STATUS_INDEX_NEW: (
      TBD_STATUS_TRACKED, False, True, True, False),
  ...
  def _au_files(self):
    for f_out in stdout(
        git('ls-files', '-v', _cwd=self.tbd_repo.root)).splitlines():
      if f_out[0] == 'h':
        yield f_out[2:].strip()
  ...
  def status(self):
    ...
    """
    for fp, git_s in self.tbd_repo.git_repo.status().items():
    ...
    if au_files:
      for fp in au_files:
        exists_in_wd = os.path.exists(os.path.join(self.tbd_repo.root, fp))
        yield self.FileStatus(
            fp, TBD_STATUS_UNTRACKED, True, exists_in_wd, True, False)
  ...
  def _status_file(self, path):
    ...
    git_st = self.tbd_repo.git_repo.status_file(_get_git_path(path))
    root = self.tbd_repo.root
    ...
    if is_au:
      ...
      f_st = self.FileStatus(
          path, TBD_STATUS_UNTRACKED, True, exists_in_wd, True, False)
    else:
  ...
  def path_is_ignored(self, path):
    ...
    return self.tbd_repo.git_repo.path_is_ignored(git_path)
  ...
  def track_file(self, path):
    ...
    tbd_st, git_st, is_au = self._status_file(path)

    if tbd_st.type == TBD_STATUS_TRACKED:
      raise ValueError('File {0} is already tracked'.format(path))
    elif tbd_st.type == TBD_STATUS_IGNORED:
      raise ValueError(
    ...
    if git_st == pygit2.GIT_STATUS_WT_NEW:  # Case (i)
      ...
      git('update-index', '--no-assume-unchanged', path,
          _cwd=self.tbd_repo.root)
    else:
      raise TbdError('File {0} in unknown status {1}'.format(path, git_st))
  ...
  def resolve_file(self, path):
    ...
    tbd_st, _, _ = self._status_file(path)
    if not tbd_st.in_conflict:
  ...
  def checkout_file(self, path, commit):
    ...
    if o.type == pygit2.GIT_OBJ_BLOB:
      ...
      # appear as modified to tbd when it shouldn't. This is also consistent
  ...
  def get_paths(self, path, commit):
    ...
    git_path = _get_git_path(path)
    tree = self.tbd_repo.git_repo[commit.tree[git_path].id]
  ...
  def diff_file(self, path):
    ...
    git_repo = self.tbd_repo.git_repo
    git_path = _get_git_path(path)
    try:
  ...
  def merge(self, src, op_cb=None):
    ...
    result, unused_ff_conf = self.tbd_repo.git_repo.merge_analysis(src.target)
    if result & pygit2.GIT_MERGE_ANALYSIS_UP_TO_DATE:
      raise TbdError('No commits to merge')
    try:
      git.merge(src, '--no-ff')
    except ErrorReturnCode as e:
      err = stderr(e)
      if not 'stash' in err:
        raise TbdError(stdout(e) + err)
      ...
      try:
        ...
        raise TbdError(stdout(e) + stderr(e))
  ...
  def merge_continue(self, op_cb=None):
    if not self.merge_in_progress:
      raise TbdError('No merge in progress, nothing to continue')
    self._state_cleanup()
    restore_fn = op_cb.restore_ok if op_cb else None
    self._safe_restore(_stash_msg_merge, restore_fn=restore_fn)
  ...
  def merge_in_progress(self):
    return self.tbd_repo._ref_exists('MERGE_HEAD')

  def abort_merge(self):
    if not self.merge_in_progress:
      raise TbdError('No merge in progress, nothing to abort')
  ...
  def _fuse_commits_fp(self):
    return self.tbd_repo._fuse_commits_fp(self)
  ...
  def fuse(self, src, ip, only=None, exclude=None, op_cb=None):
    ...
    if not any(_fuse_commits):
      raise TbdError('No commits to fuse')
    ...
    orig_head_fp = os.path.join(repo.path, 'TBD_FUSE_ORIG_HEAD')
  ...
  def fuse_continue(self, op_cb=None):
    if not self.fuse_in_progress:
      raise TbdError('No fuse in progress, nothing to continue')
  ...
  def _fuse(self, commits, op_cb=None):
    ...
    for ci in commits:
      ...
      if index.conflicts:
        ...
        raise TbdError('There are conflicts you need to resolve')
    ...
    orig_branch_ref = git_repo.lookup_reference('TBD_FUSE_ORIG_HEAD').resolve()
  ...
  def fuse_in_progress(self):
    return self.tbd_repo._ref_exists('TBD_FUSE_ORIG_HEAD')

  def abort_fuse(self, op_cb=None):
    if not self.fuse_in_progress:
      raise TbdError('No fuse in progress, nothing to abort')
    git_repo = self.tbd_repo.git_repo
    git_repo.set_head(git_repo.lookup_reference('TBD_FUSE_ORIG_HEAD').target)
  ...
  def _state_cleanup(self):
    self.tbd_repo.git_repo.state_cleanup()
    if os.path.exists(self._fuse_commits_fp):
      os.remove(self._fuse_commits_fp)
    self.tbd_repo._ref_rm('TBD_FUSE_ORIG_HEAD')
  ...
  def create_commit(self, files, msg, author=None, partials=None):
    ...
    index = self._index
    if index.conflicts:
      raise TbdError('Unresolved conflicts')
    ...
    def get_tree_and_update_index():
      ...
      def update():
        ...
        for f in files:
          ...
          if not os.path.exists(os.path.join(self.tbd_repo.root, f)):
            index.remove(git_f)
    ...
    return self.tbd_repo.git_repo[ci_oid]
  ...
  def publish(self, branch):
    ...
    try:
      ...
      assert branch.branch_name in self.tbd_repo.remotes[
      ...
      if 'Everything up-to-date' in stderr(cmd):
        raise TbdError('No commits to publish')
    except ErrorReturnCode as e:
      err_msg = stderr(e)
      if 'Updates were rejected' in err_msg:
        raise TbdError('There are changes you need to fuse/merge')
      raise TbdError(err_msg)
  ...
  def _check_op_not_in_progress(self):
    if self.merge_in_progress:
      raise TbdError('Merge in progress')
    if self.fuse_in_progress:
      raise TbdError('Fuse in progress')
...
def _stash(pattern):
  ...
  if not result:
    raise TbdError('Unexpected output of git stash: {0}'.format(out))

  return result.group(1).strip(), result.group(2).strip()

def _stash_msg(name):
  return '---tbd-{0}---'.format(name)
