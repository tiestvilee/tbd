...
"""tbd remote - List, create, edit or delete remotes."""
...
def _do_list(remotes):
  ...
  pprint.exp(
      'do tbd remote -c r r_url to add a new remote r mapping to r_url')
  pprint.exp('do tbd remote -d r to delete remote r')
  pprint.blank()
...
def _do_create(rn, ru, remotes):
  ...
  pprint.exp('to list existing remotes do tbd remote')
  pprint.exp('to remove {0} do tbd remote -d {1}'.format(rn, rn))
  return True
