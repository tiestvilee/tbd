...
"""tbd commit - Record changes in the local repository."""
...
def main(args, repo):
  ...
  if not commit_files:
    ...
    pprint.err_exp('use tbd track f if you want to track changes to file f')
    return False
...
def _auto_track(files, curr_b):
  ...
  for fp in files:
    ...
    if f.type == core.TBD_STATUS_UNTRACKED:
