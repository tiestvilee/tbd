...
"""tbd status - Show the status of files in the repo."""
...
def main(args, repo):
  ...
  for f in curr_b.status():
    if paths and (f.fp not in paths):
      continue
    if f.type == core.TBD_STATUS_TRACKED and f.modified:
      tracked_mod_list.append(f)
    elif f.type == core.TBD_STATUS_UNTRACKED:
...
def _print_tracked_mod_files(tracked_mod_list, relative_paths, repo):
  ...
  pprint.exp(
      'use tbd untrack f if you don\'t want to track changes to file f')
  pprint.exp(
      'if file f was committed before, use tbd checkout f to discard '
      'local changes')
  pprint.blank()
...
def _print_untracked_files(untracked_list, relative_paths, repo):
  ...
  pprint.exp('use tbd track f if you want to track changes to file f')
  pprint.blank()
...
def _print_conflict_exp(op):
  pprint.msg(
      ...
      'commiting'.format(op))
  pprint.exp(
      'use tbd {0} --abort to go back to the state before the {0}'.format(op))
  pprint.exp('use tbd resolve f to mark file f as resolved')
  pprint.exp('once you solved all conflicts do tbd commit to continue')
  pprint.blank()
