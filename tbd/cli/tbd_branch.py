...
"""tbd branch - List, create, edit or delete branches."""
...
def _do_list(repo, list_remote, v=False):
  ...
  pprint.exp('do tbd branch -c b to create branch b')
  pprint.exp('do tbd branch -d b to delete branch b')
  pprint.exp('do tbd switch b to switch to branch b')
...
def _do_delete(delete_b, repo):
  ...
  for b_name in delete_b:
    try:
      ...
      pprint.err(e)
      pprint.err_exp(
          'do tbd branch b to create or switch to another branch b and then '
          'tbd branch -d {0} to remove branch {0}'.format(b))
      errors_found = True
