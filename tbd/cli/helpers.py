...
def oei_fs(args, repo):
  ...
  if only:
    ...
    ret = frozenset(
        ...
        if f.type == core.TBD_STATUS_TRACKED and f.modified) # using generator expression
...
def _oei_validate(only, exclude, include, curr_b):
  ...
  only_valid = validate(
      only, lambda f: f.type == core.TBD_STATUS_UNTRACKED or (
          f.type == core.TBD_STATUS_TRACKED and f.modified),
      'File {0} is not a tracked modified or untracked file'.format)
  exclude_valid = validate(
      exclude, lambda f: f.type == core.TBD_STATUS_TRACKED and f.modified,
      'File {0} is not a tracked modified file'.format)
  include_valid = validate(
      include, lambda f: f.type == core.TBD_STATUS_UNTRACKED,
